      ...
      def merged_file
        ...
        @sources.each {|s| 
          File.open("#{@asset_path}/#{s}.#{@extension}", "r") { |f| 
            merged_file += (@extension == 'css' ? fix_css_url_paths(f.read + "\n", s) : f.read + "\n")
          }
        }
        merged_file
      end
    
      def fix_css_url_paths(source, path)
        # add path dirname to beginning of url
        return path if !URI::extract(path).empty? # don't touch if it is a uri
        dirname =  File.dirname(path)
        dirname == '.' ? source : source.gsub(/\.\.\/[^'")]+/) {|match| "#{dirname}/#{match}"}
      end
